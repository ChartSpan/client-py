...
class FHIRServer(object):
    ...
    def __init__(self, client, base_uri=None, state=None, settings=None):
        self.client = client
        self.auth = None
        self.base_uri = None
        self.aud = None
    ...
    def get_capability(self, force=False):
        ...
        or forced.
        """
        if self.settings and self.settings.get('auth_type'):
            logger.info(f'Auth type {self.settings["auth_type"]} in settings, skipping getting capability statement')
            self.auth = FHIRAuth.create(self.settings['auth_type'], self.settings)
